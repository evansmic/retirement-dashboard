...
const FIXTURE = {
  ...
  a_planStart:2026, a_planEnd:2070, a_frankDies:0, a_returnRate:0.05,
...
const D = {
  ...
  assumptions: {
    ...
    planEnd: n('a_planEnd',2070), frankDiesInSurvivor: n('a_frankDies',0),
...
check(out.D.frank.name === 'Alice', `Frank name = '${out.D.frank.name}'`);
check(out.D.moon.name === 'Bob', `Moon name = '${out.D.moon.name}'`);
check(out.D.frank.retireYear === 2033, `Frank retireYear = ${out.D.frank.retireYear}`);
check(out.D.moon.retireYear === 2037, `Moon retireYear = ${out.D.moon.retireYear}`);
check(out.D.frank.salary === 150000, `Frank salary = $${out.D.frank.salary.toLocaleString()}`);
check(out.D.moon.salary === 110000, `Moon salary = $${out.D.moon.salary.toLocaleString()}`);
check(out.D.assumptions.planStart === 2026, `planStart = ${out.D.assumptions.planStart}`);
check(out.D.moon.db_after65 === 32000, `Moon DB_after65 = $${out.D.moon.db_after65.toLocaleString()}`);
...
check(y2030 && y2030.salary_f > 150000, `2030 Frank salary (grown from $150K) = $${Math.round((y2030||{}).salary_f||0).toLocaleString()}`);
check(y2030 && y2030.salary_m > 110000, `2030 Moon salary (grown from $110K) = $${Math.round((y2030||{}).salary_m||0).toLocaleString()}`);
