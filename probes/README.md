...
| `probe_plan_file_roundtrip.js` | Local `.plan.json` wrapper/extract round-trip, raw v2 import support, dashboard-style annual CPP/OAS import back to monthly form fields, blank Person 2 normalization, and malformed/unsupported file rejection (Sprint 3 S3-01, Sprint 4 smoke coverage) | 17 |
| `probe_single_plan_file_dashboard.js` | Dashboard regression for malformed single-person `.plan.json` exports where blank Person 2 was saved as a placeholder and zero retirement fields inflated spending/chart data, including cash-wedge source reconciliation | 10 |
| `probe_engine_bridge_parity.js` | Migration bridge parity for the interim `runSimulation(plan, cfg)` boundary, covering a bundled preset and Larry single-person plan | 10 |
| `probe_simulation_engine_parity.js` | Extracted `engine/simulation_engine.js` parity against the dashboard-backed bridge for a bundled preset and Larry single-person plan | 12 |
