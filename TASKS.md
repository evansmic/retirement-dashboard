...
Goal: recover or replace the deferred full `npm test` path on the current low-storage machine. The repo now has `npm run test:full:low-storage`, a low-storage batch runner with per-batch timeout diagnostics. The bounded optimizer emits `fullSuiteRecoveryPlan`, which records the replacement command, required focused/build checks, and the isolated long pole.

Package doc: [`docs/sprint_3688_3707_full_suite_recovery_plan.md`](docs/sprint_3688_3707_full_suite_recovery_plan.md).

### S3688-S3707 Completed Path

- Added `scripts/run_full_suite_low_storage.mjs`.
- Added `npm run test:full:low-storage`.
- Added per-batch timeout diagnostics via `TEST_BATCH_TIMEOUT_MS`.
- Added `OptimizerFullSuiteRecoveryPlan` and recovery rows.
- Rendered full-suite recovery in Results Details.
- Confirmed the new runner advances through most batches and isolates the unresolved long pole to `app/src/engine/examplePlanOptimizerReadiness.test.ts`.

### S3688-S3707 Definition Of Done

- `npm run test:full:low-storage` exists and discovers test files dynamically.
- The runner executes test files in low-storage batches and reports timeout batches instead of hanging indefinitely.
- Details shows the full-suite recovery plan.
- The isolated long-pole test is named for the next repair package.
...
Goal: implement the private pilot requirements named by release narrowing. The bounded optimizer now emits `privatePilotRequirements` with opt-in copy requirements, privacy boundaries, stop conditions, tester limits, and evidence rows for deciding whether public output can later be reconsidered.

Package doc: [`docs/sprint_3668_3687_private_pilot_requirements.md`](docs/sprint_3668_3687_private_pilot_requirements.md).

### S3668-S3687 Completed Path

- Added `OptimizerPrivatePilotRequirements` and requirement/evidence rows.
- Defined opt-in copy, privacy boundary, stop condition, tester-limit, and evidence criteria requirements.
- Limited private pilot scope to a maximum of five private opt-in households.
...
### S3668-S3687 Definition Of Done

- Details shows the private pilot requirements packet.
- Private pilot requirements name opt-in, privacy, stop-condition, tester-limit, and evidence criteria.
- Real-data use remains explicit opt-in only, public sharing remains blocked, and no tester data is collected by this package.
...
- Private pilot requirements are not persisted into `.plan.json`.
- Focused optimizer tests, UI structure tests, result selector tests, and production build pass before commit.

## Previous Package — S3648-S3667: Public Optimizer Release Narrowing
...
Goal: make the live assumption lab intentional enough for beta review. Details controls now distinguish pending slider changes from applied preview runs, show the adjusted assumption being tested, and make the current/optimal/alternative comparison cards easier to read against the current plan.

Package doc: [`docs/sprint_3608_3627_assumption_lab_comparison_polish.md`](docs/sprint_3608_3627_assumption_lab_comparison_polish.md).

### S3608-S3627 Completed Path

- Split assumption lab state into pending draft and applied preview assumptions.
- Added explicit Apply and Reset controls instead of rerunning on every slider tick.
- Added pending/applied assumption summary labels.
- Kept progress feedback for applied preview reruns.
- Added comparison deltas against the current plan for optimal and alternative slots.
- Updated assumption lab boundary copy now that temporary working-copy reruns exist.

### S3608-S3627 Definition Of Done

- Details controls do not rerun on every slider movement.
- Users can see pending adjustment versus applied preview assumption.
- Comparison cards show current-plan deltas for ending portfolio and lifetime tax.
...
Goal: define how a future hand-written static mock surface must remain removable before production UI work. The tester-only surface now shows removal units, data/route/release boundaries, and removal blockers while keeping shared selectors, saved schema dependencies, engine output dependencies, report dependencies, CSV dependencies, production routes, rendered rows, calculations, exports, and schema changes blocked.

Non-scope: saved plan schema changes, engine output schema changes, persisted optimizer output, printable report output changes, CSV output changes, `.plan.json` files, production UI promotion, final advice-like copy, tax-bracket instructions, exportable sequencing output, final annual instructions, new optimizer behavior, in-app feedback collection, feedback scoring, approval/unlock logic for rendered rows, issue creation, cleanup task creation, model repair automation, annual instruction calculations, annual instruction prototype implementation, mapping functions, generated instruction rows, static row mock implementation, static mock rows, rendered fixture rows, fixture calculations, fixture export, copy generation, timeline approval logic, or changed public-readiness estimates.

Package doc: [`docs/sprint_2888_2907_annual_instruction_static_mock_surface_removal_contract.md`](docs/sprint_2888_2907_annual_instruction_static_mock_surface_removal_contract.md).

### S2888-S2907 Completed Path

- **S2888-S2892 — Removal unit batch.** Added a rule that any future mock surface must be grouped for removal in one scoped change.
- **S2893-S2897 — Boundary dependency batch.** Added data, route, and release boundaries that keep future mock content out of shared outputs.
- **S2898-S2902 — Source guard batch.** Added structure checks that block selector, saved schema, engine output, report, CSV, production route, and release dependencies.
- **S2903-S2907 — Verification and closeout.** Ran UI structure checks, focused optimizer tests, plan-file tests, production build, file guards, and browser verification.

### S2888-S2907 Definition Of Done

- Tiny tester surface remains Details-only and tester-only.
- Static mock surface removal contract is visible.
- Removal unit, data boundary, route boundary, and release boundary are visible.
- Shared selector, saved schema, engine output, report, CSV, and production route dependencies remain blocked.
...
## Previous Package — S2868-S2887: Annual Instruction Static Mock Surface Layout Contract
