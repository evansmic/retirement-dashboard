...
import {
  ...
  selectMonthlyCapacityFundingTraceImplementationContract,
  selectMonthlyCapacityFundingTraceImplementationExampleReadiness,
  selectMonthlyCapacityFundingTraceImplementationGuardrails,
  ...
  selectMonthlyCapacityFundingTracePlanningGuardrails,
  selectMonthlyCapacityFundingTracePlanningPackageCloseout,
  selectMonthlyCapacityFundingTracePlanningPlan,
  selectMonthlyCapacityFundingTracePlanningReadiness,
  selectMonthlyCapacityFundingTracePlanningSummary,
  ...
  selectMonthlyCapacityRecommendationRuntimeDryRun,
  selectMonthlyCapacityRecommendationRuntimeDryRunAudit,
  selectMonthlyCapacityRecommendationRuntimeExecutionBlockReview,
  selectMonthlyCapacityRecommendationRuntimeExecutionCloseout,
  selectMonthlyCapacityRecommendationRuntimeExecutionExampleReadiness,
  selectMonthlyCapacityRecommendationRuntimeExecutionPackageCloseout,
  selectMonthlyCapacityRecommendationRuntimeExecutionPlan,
  selectMonthlyCapacityRecommendationRuntimeExecutionReadiness,
  selectMonthlyCapacityRecommendationRuntimeExecutionSummary,
  selectMonthlyCapacityRecommendationRuntimeCloseout,
  selectMonthlyCapacityRecommendationRuntimeExampleReadiness,
  selectMonthlyCapacityRecommendationRuntimePackageCloseout,
  selectMonthlyCapacityRecommendationRuntimeSelection,
  selectMonthlyCapacityRecommendationRuntimeSelectionAudit,
  selectMonthlyCapacityRecommendationRuntimeSummary,
...
function buildMonthlyRecommendationExecutionRuntime(
  result: SimulationResult,
  plan: V2PlanPayload,
  comparisons: Record<string, SimulationResult> = {}
) {
  const runtime = buildMonthlyCandidateRuntime(result, plan, comparisons);
  const handoff = selectMonthlyCapacityRuntimeCandidateSimulationHandoff(runtime.variants);
  const simulationSet = selectMonthlyCapacityRuntimeCandidateSimulations(runtime.variants, handoff, () => result);
  ...
  const recommendationCloseout = selectMonthlyCapacityRecommendationPlanningCloseout(recommendationPlan, copyPolicy);
  const executionPlan = selectMonthlyCapacityRecommendationRuntimeExecutionPlan(recommendationCloseout, recommendationPlan);
  const recommendationDryRun = selectMonthlyCapacityRecommendationRuntimeDryRun(executionPlan);
  const executionCloseout = selectMonthlyCapacityRecommendationRuntimeExecutionCloseout(executionPlan, recommendationDryRun);
  const blockReview = selectMonthlyCapacityRecommendationRuntimeExecutionBlockReview(executionPlan, recommendationDryRun);
  const exampleReadiness = selectMonthlyCapacityRecommendationRuntimeExecutionExampleReadiness([executionCloseout]);
  const packageCloseout = selectMonthlyCapacityRecommendationRuntimeExecutionPackageCloseout(
    executionCloseout,
    blockReview,
    exampleReadiness
  );

  return { ranking, executionCloseout, packageCloseout };
}

function buildMonthlyFundingTracePlanningRuntime(result: SimulationResult, plan: V2PlanPayload) {
  const runtime = buildMonthlyRecommendationExecutionRuntime(result, plan);
  const selection = selectMonthlyCapacityRecommendationRuntimeSelection(runtime.packageCloseout, runtime.ranking);
  const recommendationCloseout = selectMonthlyCapacityRecommendationRuntimeCloseout(selection);
  const tracePlan = selectMonthlyCapacityFundingTracePlanningPlan(recommendationCloseout);
  const traceCloseout = selectMonthlyCapacityFundingTracePlanningCloseout(tracePlan);
  const traceExampleReadiness = selectMonthlyCapacityFundingTracePlanningExampleReadiness([traceCloseout]);
  const tracePackageCloseout = selectMonthlyCapacityFundingTracePlanningPackageCloseout(
    traceCloseout,
    traceExampleReadiness
  );
...
describe('result selectors', () => {
  ...
  it('plans recommendation runtime execution without selecting a recommendation', () => {
    const runtime = buildMonthlyCandidateRuntime(fixture, planFixture);
    const handoff = selectMonthlyCapacityRuntimeCandidateSimulationHandoff(runtime.variants);
    const simulationSet = selectMonthlyCapacityRuntimeCandidateSimulations(runtime.variants, handoff, () => fixture);
    const scoringPlan = selectMonthlyCapacityScoringExecutionPlan(simulationSet);
    ...
    expect(executionPlan.boundary).toContain('without selecting recommendations');
    expect(createPlanFile(planFixture).plan).not.toHaveProperty('monthlyCapacityRecommendationRuntimeExecutionPlan');
  });

  it('dry-runs recommendation runtime execution without output', () => {
    const runtime = buildMonthlyCandidateRuntime(fixture, planFixture);
    const handoff = selectMonthlyCapacityRuntimeCandidateSimulationHandoff(runtime.variants);
    const simulationSet = selectMonthlyCapacityRuntimeCandidateSimulations(runtime.variants, handoff, () => fixture);
    const scoringPlan = selectMonthlyCapacityScoringExecutionPlan(simulationSet);
    ...
    expect(recommendationDryRun).toMatchObject({
      status: 'readyForFutureExecution',
    ...
    expect(recommendationDryRun.boundary).toContain('without selecting recommendations');
    expect(createPlanFile(planFixture).plan).not.toHaveProperty('monthlyCapacityRecommendationRuntimeDryRun');
  });

  it('audits recommendation runtime dry run so it cannot become output', () => {
    const runtime = buildMonthlyCandidateRuntime(fixture, planFixture);
    const handoff = selectMonthlyCapacityRuntimeCandidateSimulationHandoff(runtime.variants);
    const simulationSet = selectMonthlyCapacityRuntimeCandidateSimulations(runtime.variants, handoff, () => fixture);
    const scoringPlan = selectMonthlyCapacityScoringExecutionPlan(simulationSet);
    ...
    const executionPlan = selectMonthlyCapacityRecommendationRuntimeExecutionPlan(recommendationCloseout, recommendationPlan);
    const recommendationDryRun = selectMonthlyCapacityRecommendationRuntimeDryRun(executionPlan);
    const audit = selectMonthlyCapacityRecommendationRuntimeDryRunAudit(recommendationDryRun);
    ...
    expect(audit.boundary).toContain('did not select recommendations');
    expect(createPlanFile(planFixture).plan).not.toHaveProperty('monthlyCapacityRecommendationRuntimeDryRunAudit');
  });

  it('summarizes recommendation runtime execution readiness without execution', () => {
    const runtime = buildMonthlyCandidateRuntime(fixture, planFixture);
    const handoff = selectMonthlyCapacityRuntimeCandidateSimulationHandoff(runtime.variants);
    const simulationSet = selectMonthlyCapacityRuntimeCandidateSimulations(runtime.variants, handoff, () => fixture);
    const scoringPlan = selectMonthlyCapacityScoringExecutionPlan(simulationSet);
    ...
    const executionPlan = selectMonthlyCapacityRecommendationRuntimeExecutionPlan(recommendationCloseout, recommendationPlan);
    const recommendationDryRun = selectMonthlyCapacityRecommendationRuntimeDryRun(executionPlan);
    const executionReadiness = selectMonthlyCapacityRecommendationRuntimeExecutionReadiness(executionPlan, recommendationDryRun);

    expect(executionReadiness).toMatchObject({
      status: 'readyForFutureExecution',
      topCandidateId: 'baseline',
      plannedCheckCount: 5,
      recommendationCandidateId: null
    });
    expect(executionReadiness.boundary).toContain('without selecting recommendations');
    expect(createPlanFile(planFixture).plan).not.toHaveProperty('monthlyCapacityRecommendationRuntimeExecutionReadiness');
  });

  it('summarizes recommendation runtime execution planning', () => {
    const runtime = buildMonthlyCandidateRuntime(fixture, planFixture);
    const handoff = selectMonthlyCapacityRuntimeCandidateSimulationHandoff(runtime.variants);
    const simulationSet = selectMonthlyCapacityRuntimeCandidateSimulations(runtime.variants, handoff, () => fixture);
    const scoringPlan = selectMonthlyCapacityScoringExecutionPlan(simulationSet);
    ...
    const executionPlan = selectMonthlyCapacityRecommendationRuntimeExecutionPlan(recommendationCloseout, recommendationPlan);
    const recommendationDryRun = selectMonthlyCapacityRecommendationRuntimeDryRun(executionPlan);
    const summary = selectMonthlyCapacityRecommendationRuntimeExecutionSummary(executionPlan, recommendationDryRun);
    ...
    expect(summary).toMatchObject({
      ...
      recommendationCandidateId: null,
      plannedCheckCount: 5,
      nextBroadStep: 'recommendationRuntimeExecution'
    });
    expect(summary.boundary).toContain('without selecting recommendations');
    expect(createPlanFile(planFixture).plan).not.toHaveProperty('monthlyCapacityRecommendationRuntimeExecutionSummary');
  });

  it('closes out recommendation runtime execution planning without selecting output', () => {
    const runtime = buildMonthlyCandidateRuntime(fixture, planFixture);
    const handoff = selectMonthlyCapacityRuntimeCandidateSimulationHandoff(runtime.variants);
    const simulationSet = selectMonthlyCapacityRuntimeCandidateSimulations(runtime.variants, handoff, () => fixture);
    const scoringPlan = selectMonthlyCapacityScoringExecutionPlan(simulationSet);
    ...
    const executionPlan = selectMonthlyCapacityRecommendationRuntimeExecutionPlan(recommendationCloseout, recommendationPlan);
    const recommendationDryRun = selectMonthlyCapacityRecommendationRuntimeDryRun(executionPlan);
    const closeout = selectMonthlyCapacityRecommendationRuntimeExecutionCloseout(executionPlan, recommendationDryRun);
    ...
    expect(closeout.stillDeferred).toContain('recommendations');
    expect(closeout.boundary).toContain('but no recommendation');
    expect(createPlanFile(planFixture).plan).not.toHaveProperty('monthlyCapacityRecommendationRuntimeExecutionCloseout');
  });

  it('reviews recommendation runtime execution blockers without creating output', () => {
    const runtime = buildMonthlyCandidateRuntime(fixture, planFixture);
    const handoff = selectMonthlyCapacityRuntimeCandidateSimulationHandoff(runtime.variants);
    const simulationSet = selectMonthlyCapacityRuntimeCandidateSimulations(runtime.variants, handoff, () => fixture);
    const scoringPlan = selectMonthlyCapacityScoringExecutionPlan(simulationSet);
    ...
    const executionPlan = selectMonthlyCapacityRecommendationRuntimeExecutionPlan(recommendationCloseout, recommendationPlan);
    const recommendationDryRun = selectMonthlyCapacityRecommendationRuntimeDryRun(executionPlan);
    const clearReview = selectMonthlyCapacityRecommendationRuntimeExecutionBlockReview(executionPlan, recommendationDryRun);
    const blockedPlan = { ...executionPlan, status: 'blocked' as const, topCandidateId: null };
    const blockedDryRun = selectMonthlyCapacityRecommendationRuntimeDryRun(blockedPlan);
    const blockedReview = selectMonthlyCapacityRecommendationRuntimeExecutionBlockReview(blockedPlan, blockedDryRun);

    expect(clearReview).toMatchObject({
      status: 'clear',
      blockedReasonIds: [],
      recommendationCandidateId: null,
      saved: false
    ...
    expect(blockedReview.blockedReasonIds).toEqual([
      'missingPlanningCloseout',
      'missingTopCandidate',
      'dryRunBlocked',
      'readinessBlocked'
    ]);
    expect(blockedReview.boundary).toContain('without selecting recommendations');
    expect(createPlanFile(planFixture).plan).not.toHaveProperty('monthlyCapacityRecommendationRuntimeExecutionBlockReview');
  });

  it('checks recommendation runtime execution examples without selecting output', () => {
    const runtime = buildMonthlyCandidateRuntime(fixture, planFixture);
    const handoff = selectMonthlyCapacityRuntimeCandidateSimulationHandoff(runtime.variants);
    const simulationSet = selectMonthlyCapacityRuntimeCandidateSimulations(runtime.variants, handoff, () => fixture);
    const scoringPlan = selectMonthlyCapacityScoringExecutionPlan(simulationSet);
    ...
    const recommendationDryRun = selectMonthlyCapacityRecommendationRuntimeDryRun(executionPlan);
    const closeout = selectMonthlyCapacityRecommendationRuntimeExecutionCloseout(executionPlan, recommendationDryRun);
    const readinessMatrix = selectMonthlyCapacityRecommendationRuntimeExecutionExampleReadiness([closeout, closeout]);
    const blockedMatrix = selectMonthlyCapacityRecommendationRuntimeExecutionExampleReadiness([
      closeout,
      { ...closeout, status: 'blocked' as const }
    ]);

    expect(readinessMatrix).toMatchObject({
      status: 'ready',
      exampleCount: 2,
      readyExampleCount: 2,
    ...
    expect(blockedMatrix).toMatchObject({
      status: 'blocked',
      exampleCount: 2,
      readyExampleCount: 1,
    ...
    expect(readinessMatrix.boundary).toContain('without selecting recommendations');
    expect(createPlanFile(planFixture).plan).not.toHaveProperty('monthlyCapacityRecommendationRuntimeExecutionExampleReadiness');
  });

  it('closes out the recommendation runtime execution planning package', () => {
    const runtime = buildMonthlyCandidateRuntime(fixture, planFixture);
    const handoff = selectMonthlyCapacityRuntimeCandidateSimulationHandoff(runtime.variants);
    const simulationSet = selectMonthlyCapacityRuntimeCandidateSimulations(runtime.variants, handoff, () => fixture);
    const scoringPlan = selectMonthlyCapacityScoringExecutionPlan(simulationSet);
    ...
    const closeout = selectMonthlyCapacityRecommendationRuntimeExecutionCloseout(executionPlan, recommendationDryRun);
    const blockReview = selectMonthlyCapacityRecommendationRuntimeExecutionBlockReview(executionPlan, recommendationDryRun);
    const exampleReadiness = selectMonthlyCapacityRecommendationRuntimeExecutionExampleReadiness([closeout]);
    const packageCloseout = selectMonthlyCapacityRecommendationRuntimeExecutionPackageCloseout(
      closeout,
      blockReview,
      exampleReadiness
    );
    const blockedPackageCloseout = selectMonthlyCapacityRecommendationRuntimeExecutionPackageCloseout(
      closeout,
      blockReview,
      selectMonthlyCapacityRecommendationRuntimeExecutionExampleReadiness([{ ...closeout, status: 'blocked' as const }])
    );
    ...
    expect(packageCloseout.stillDeferred).toContain('recommendations');
    expect(packageCloseout.boundary).toContain('without selecting recommendations');
    ...
    expect(createPlanFile(planFixture).plan).not.toHaveProperty('monthlyCapacityRecommendationRuntimeExecutionPackageCloseout');
  });

  it('selects one runtime recommendation candidate without saving or tracing output', () => {
    const runtime = buildMonthlyRecommendationExecutionRuntime(fixture, planFixture);
    const selection = selectMonthlyCapacityRecommendationRuntimeSelection(runtime.packageCloseout, runtime.ranking);
    const blockedSelection = selectMonthlyCapacityRecommendationRuntimeSelection(
      { ...runtime.packageCloseout, status: 'blocked' as const },
      runtime.ranking
    );
    ...
    expect(selection.totalScore).toBeGreaterThanOrEqual(0);
    expect(selection.copy).toContain('not financial advice');
    expect(selection.boundary).toContain('without saving output');
    ...
    expect(createPlanFile(planFixture).plan).not.toHaveProperty('monthlyCapacityRecommendationRuntimeSelection');
  });

  it('audits runtime recommendation selection boundaries', () => {
    const runtime = buildMonthlyRecommendationExecutionRuntime(fixture, planFixture);
    const selection = selectMonthlyCapacityRecommendationRuntimeSelection(runtime.packageCloseout, runtime.ranking);
    const audit = selectMonthlyCapacityRecommendationRuntimeSelectionAudit(selection);
    const advisoryCopyAudit = selectMonthlyCapacityRecommendationRuntimeSelectionAudit({
      ...selection,
      copy: 'Use this path.'
    });
    ...
    expect(audit).toMatchObject({
      ...
      annualSequencingCount: 0,
      uiPresentationCount: 0,
      nonAdvisoryCopy: true
    });
    expect(audit.boundary).toContain('allows one selected candidate');
    expect(advisoryCopyAudit).toMatchObject({
      status: 'block',
      selectedCandidateCount: 1,
      nonAdvisoryCopy: false
    });
    expect(createPlanFile(planFixture).plan).not.toHaveProperty('monthlyCapacityRecommendationRuntimeSelectionAudit');
  });

  it('summarizes runtime recommendation selection without trace or UI output', () => {
    const runtime = buildMonthlyRecommendationExecutionRuntime(fixture, planFixture);
    const selection = selectMonthlyCapacityRecommendationRuntimeSelection(runtime.packageCloseout, runtime.ranking);
    const summary = selectMonthlyCapacityRecommendationRuntimeSummary(selection);
    const blockedSummary = selectMonthlyCapacityRecommendationRuntimeSummary({ ...selection, status: 'blocked' as const, recommendationCandidateId: null });
    ...
    expect(summary).toMatchObject({
      ...
      source: 'topRankedCandidate',
      nonAdvisoryCopy: true,
      nextBroadStep: 'fundingTracePlanning',
      saved: false
    });
    ...
    expect(createPlanFile(planFixture).plan).not.toHaveProperty('monthlyCapacityRecommendationRuntimeSummary');
  });

  it('closes out runtime recommendation selection without expanding scope', () => {
    const runtime = buildMonthlyRecommendationExecutionRuntime(fixture, planFixture);
    const selection = selectMonthlyCapacityRecommendationRuntimeSelection(runtime.packageCloseout, runtime.ranking);
    const closeout = selectMonthlyCapacityRecommendationRuntimeCloseout(selection);

    expect(closeout).toMatchObject({
      status: 'complete',
      headline: 'Runtime recommendation selection is complete.',
      recommendationCandidateId: 'baseline',
    ...
    expect(closeout.stillDeferred).not.toContain('recommendations');
    expect(closeout.boundary).toContain('one modelled recommendation candidate is selected');
    expect(createPlanFile(planFixture).plan).not.toHaveProperty('monthlyCapacityRecommendationRuntimeCloseout');
  });

  it('checks runtime recommendation examples without persistence', () => {
    const runtime = buildMonthlyRecommendationExecutionRuntime(fixture, planFixture);
    const selection = selectMonthlyCapacityRecommendationRuntimeSelection(runtime.packageCloseout, runtime.ranking);
    const closeout = selectMonthlyCapacityRecommendationRuntimeCloseout(selection);
    const readiness = selectMonthlyCapacityRecommendationRuntimeExampleReadiness([closeout, closeout]);
    const blockedReadiness = selectMonthlyCapacityRecommendationRuntimeExampleReadiness([
      closeout,
    ...
    expect(blockedReadiness).toMatchObject({
      status: 'blocked',
      exampleCount: 2,
      selectedExampleCount: 1,
      blockedExampleCount: 1,
      recommendationCandidateIds: ['baseline'],
      saved: false
    });
    expect(readiness.boundary).toContain('without saving output');
    expect(createPlanFile(planFixture).plan).not.toHaveProperty('monthlyCapacityRecommendationRuntimeExampleReadiness');
  });

  it('closes out the runtime recommendation execution package', () => {
    const runtime = buildMonthlyRecommendationExecutionRuntime(fixture, planFixture);
    const selection = selectMonthlyCapacityRecommendationRuntimeSelection(runtime.packageCloseout, runtime.ranking);
    const closeout = selectMonthlyCapacityRecommendationRuntimeCloseout(selection);
    const readiness = selectMonthlyCapacityRecommendationRuntimeExampleReadiness([closeout]);
    const packageCloseout = selectMonthlyCapacityRecommendationRuntimePackageCloseout(closeout, readiness);
    const blockedPackageCloseout = selectMonthlyCapacityRecommendationRuntimePackageCloseout(
      closeout,
      selectMonthlyCapacityRecommendationRuntimeExampleReadiness([{ ...closeout, status: 'blocked' as const }])
    );
    ...
    expect(packageCloseout.stillDeferred).toEqual([
      'optimizerSearch',
      'savedOptimizerOutput',
      'fundingTrace',
      'accountInstructions',
    ...
    expect(packageCloseout.boundary).toContain('one modelled recommendation candidate can be selected');
    expect(createPlanFile(planFixture).plan).not.toHaveProperty('monthlyCapacityRecommendationRuntimePackageCloseout');
  });

  it('plans funding trace inputs without creating a trace', () => {
    const runtime = buildMonthlyRecommendationExecutionRuntime(fixture, planFixture);
    const selection = selectMonthlyCapacityRecommendationRuntimeSelection(runtime.packageCloseout, runtime.ranking);
    const recommendationCloseout = selectMonthlyCapacityRecommendationRuntimeCloseout(selection);
    const tracePlan = selectMonthlyCapacityFundingTracePlanningPlan(recommendationCloseout);
    const blockedPlan = selectMonthlyCapacityFundingTracePlanningPlan({
      ...recommendationCloseout,
      status: 'blocked' as const,
      recommendationCandidateId: null
    });
    ...
    expect(tracePlan.rows.find((row) => row.id === 'taxCaveats')).toMatchObject({ status: 'review' });
    expect(tracePlan.boundary).toContain('without producing a funding trace');
    ...
    expect(createPlanFile(planFixture).plan).not.toHaveProperty('monthlyCapacityFundingTracePlanningPlan');
  });

  it('checks funding trace planning guardrails and readiness', () => {
    const runtime = buildMonthlyRecommendationExecutionRuntime(fixture, planFixture);
    const selection = selectMonthlyCapacityRecommendationRuntimeSelection(runtime.packageCloseout, runtime.ranking);
    const recommendationCloseout = selectMonthlyCapacityRecommendationRuntimeCloseout(selection);
    const tracePlan = selectMonthlyCapacityFundingTracePlanningPlan(recommendationCloseout);
    const guardrails = selectMonthlyCapacityFundingTracePlanningGuardrails(tracePlan);
    const readiness = selectMonthlyCapacityFundingTracePlanningReadiness(tracePlan, guardrails);

    expect(guardrails.map((guardrail) => guardrail.status)).toEqual(['pass', 'pass', 'pass', 'pass', 'pass', 'pass']);
    expect(readiness).toMatchObject({
      status: 'readyForFutureTracePlanning',
      reviewRowIds: ['taxCaveats'],
      blockedRowIds: [],
      passedGuardrailIds: [
        'noAccountByAccountAdvice',
        'noAnnualSequencing',
        'noSavedTrace',
        'noUiPresentation',
        'taxCaveatsRequired',
    ...
    expect(readiness.boundary).toContain('without producing a trace');
    expect(createPlanFile(planFixture).plan).not.toHaveProperty('monthlyCapacityFundingTracePlanningReadiness');
  });

  it('audits funding trace planning without creating trace output', () => {
    const runtime = buildMonthlyRecommendationExecutionRuntime(fixture, planFixture);
    const selection = selectMonthlyCapacityRecommendationRuntimeSelection(runtime.packageCloseout, runtime.ranking);
    const recommendationCloseout = selectMonthlyCapacityRecommendationRuntimeCloseout(selection);
    const tracePlan = selectMonthlyCapacityFundingTracePlanningPlan(recommendationCloseout);
    const audit = selectMonthlyCapacityFundingTracePlanningAudit(tracePlan);
    ...
    expect(audit.boundary).toContain('did not create a trace');
    expect(createPlanFile(planFixture).plan).not.toHaveProperty('monthlyCapacityFundingTracePlanningAudit');
  });

  it('summarizes funding trace planning without trace implementation', () => {
    const runtime = buildMonthlyRecommendationExecutionRuntime(fixture, planFixture);
    const selection = selectMonthlyCapacityRecommendationRuntimeSelection(runtime.packageCloseout, runtime.ranking);
    const recommendationCloseout = selectMonthlyCapacityRecommendationRuntimeCloseout(selection);
    const tracePlan = selectMonthlyCapacityFundingTracePlanningPlan(recommendationCloseout);
    const summary = selectMonthlyCapacityFundingTracePlanningSummary(tracePlan);
    const blockedSummary = selectMonthlyCapacityFundingTracePlanningSummary(
      selectMonthlyCapacityFundingTracePlanningPlan({
        ...recommendationCloseout,
        status: 'blocked' as const,
        recommendationCandidateId: null
    ...
    expect(summary).toMatchObject({
      ...
      sourceGroupCount: 6,
      reviewRowIds: ['taxCaveats'],
      nextBroadStep: 'fundingTraceImplementationPlanning',
      fundingTrace: null,
      saved: false
    });
    expect(summary.boundary).toContain('without producing a funding trace');
    expect(blockedSummary).toMatchObject({
      status: 'blocked',
      recommendationCandidateId: null,
      nextBroadStep: 'capacityInputsFirst',
    ...
    expect(createPlanFile(planFixture).plan).not.toHaveProperty('monthlyCapacityFundingTracePlanningSummary');
  });

  it('closes out funding trace planning without implementing a trace', () => {
    const runtime = buildMonthlyRecommendationExecutionRuntime(fixture, planFixture);
    const selection = selectMonthlyCapacityRecommendationRuntimeSelection(runtime.packageCloseout, runtime.ranking);
    const recommendationCloseout = selectMonthlyCapacityRecommendationRuntimeCloseout(selection);
    const tracePlan = selectMonthlyCapacityFundingTracePlanningPlan(recommendationCloseout);
    const closeout = selectMonthlyCapacityFundingTracePlanningCloseout(tracePlan);
    const blockedCloseout = selectMonthlyCapacityFundingTracePlanningCloseout(
      selectMonthlyCapacityFundingTracePlanningPlan({
        ...recommendationCloseout,
        status: 'blocked' as const,
        recommendationCandidateId: null
    ...
    expect(closeout.boundary).toContain('trace implementation');
    expect(blockedCloseout).toMatchObject({
      status: 'blocked',
      recommendationCandidateId: null,
      nextBroadStep: 'capacityInputsFirst'
    });
    expect(createPlanFile(planFixture).plan).not.toHaveProperty('monthlyCapacityFundingTracePlanningCloseout');
  });

  it('checks funding trace planning examples without trace output', () => {
    const runtime = buildMonthlyRecommendationExecutionRuntime(fixture, planFixture);
    const selection = selectMonthlyCapacityRecommendationRuntimeSelection(runtime.packageCloseout, runtime.ranking);
    const recommendationCloseout = selectMonthlyCapacityRecommendationRuntimeCloseout(selection);
    const tracePlan = selectMonthlyCapacityFundingTracePlanningPlan(recommendationCloseout);
    const closeout = selectMonthlyCapacityFundingTracePlanningCloseout(tracePlan);
    const readiness = selectMonthlyCapacityFundingTracePlanningExampleReadiness([closeout, closeout]);
    const blockedReadiness = selectMonthlyCapacityFundingTracePlanningExampleReadiness([
      closeout,
      { ...closeout, status: 'blocked' as const, recommendationCandidateId: null }
    ]);
    ...
    expect(readiness.boundary).toContain('without producing a trace');
    expect(createPlanFile(planFixture).plan).not.toHaveProperty('monthlyCapacityFundingTracePlanningExampleReadiness');
  });

  it('closes out the funding trace planning package', () => {
    const runtime = buildMonthlyRecommendationExecutionRuntime(fixture, planFixture);
    const selection = selectMonthlyCapacityRecommendationRuntimeSelection(runtime.packageCloseout, runtime.ranking);
    const recommendationCloseout = selectMonthlyCapacityRecommendationRuntimeCloseout(selection);
    const tracePlan = selectMonthlyCapacityFundingTracePlanningPlan(recommendationCloseout);
    const closeout = selectMonthlyCapacityFundingTracePlanningCloseout(tracePlan);
    const exampleReadiness = selectMonthlyCapacityFundingTracePlanningExampleReadiness([closeout]);
    const packageCloseout = selectMonthlyCapacityFundingTracePlanningPackageCloseout(closeout, exampleReadiness);
    const blockedPackageCloseout = selectMonthlyCapacityFundingTracePlanningPackageCloseout(
      closeout,
      selectMonthlyCapacityFundingTracePlanningExampleReadiness([{ ...closeout, status: 'blocked' as const }])
    );
    ...
    expect(packageCloseout.boundary).toContain('without producing a trace');
    expect(createPlanFile(planFixture).plan).not.toHaveProperty('monthlyCapacityFundingTracePlanningPackageCloseout');
  });

  it('plans the funding trace implementation contract without trace content', () => {
    const runtime = buildMonthlyFundingTracePlanningRuntime(fixture, planFixture);
    const contract = selectMonthlyCapacityFundingTraceImplementationContract(
      runtime.tracePackageCloseout,
      runtime.tracePlan
    );
    const blockedContract = selectMonthlyCapacityFundingTraceImplementationContract(
      { ...runtime.tracePackageCloseout, status: 'blocked' as const, recommendationCandidateId: null },
      runtime.tracePlan
    );
    ...
    expect(contract).toMatchObject({
      ...
      plannedSourceGroups: ['guaranteedIncome', 'registeredAssets', 'taxFreeAssets', 'taxableAssets', 'cashReserve', 'homeEquityReview'],
      plannedTrace: null,
      saved: false,
      accountInstruction: null,
      annualSequencing: null,
    ...
    expect(contract.boundary).toContain('without producing a trace');
    ...
    expect(createPlanFile(planFixture).plan).not.toHaveProperty('monthlyCapacityFundingTraceImplementationContract');
  });

  it('checks funding trace implementation guardrails and readiness', () => {
    const runtime = buildMonthlyFundingTracePlanningRuntime(fixture, planFixture);
    const contract = selectMonthlyCapacityFundingTraceImplementationContract(
      runtime.tracePackageCloseout,
      runtime.tracePlan
    );
    const guardrails = selectMonthlyCapacityFundingTraceImplementationGuardrails(contract);
    const readiness = selectMonthlyCapacityFundingTraceImplementationReadiness(contract, guardrails);

    expect(guardrails.map((guardrail) => guardrail.status)).toEqual(['pass', 'pass', 'pass', 'pass', 'pass', 'pass']);
    expect(readiness).toMatchObject({
      status: 'readyForFutureImplementationPlanning',
      plannedSectionCount: 5,
      plannedSourceGroupCount: 6,
      passedGuardrailIds: [
        'noTraceOutputYet',
        'noSavedOutput',
        'sourceGroupsOnly',
    ...
    expect(readiness.boundary).toContain('without producing a trace');
    expect(createPlanFile(planFixture).plan).not.toHaveProperty('monthlyCapacityFundingTraceImplementationReadiness');
  });

  it('audits funding trace implementation planning without trace content', () => {
    const runtime = buildMonthlyFundingTracePlanningRuntime(fixture, planFixture);
    const contract = selectMonthlyCapacityFundingTraceImplementationContract(
      runtime.tracePackageCloseout,
      runtime.tracePlan
    );
    const audit = selectMonthlyCapacityFundingTraceImplementationAudit(contract);

    expect(audit).toMatchObject({
      status: 'pass',
      plannedTraceOutputCount: 0,
      savedOutputCount: 0,
      accountInstructionCount: 0,
      annualSequencingCount: 0,
      uiPresentationCount: 0,
    ...
    expect(audit.boundary).toContain('did not produce trace content');
    expect(createPlanFile(planFixture).plan).not.toHaveProperty('monthlyCapacityFundingTraceImplementationAudit');
  });

  it('summarizes funding trace implementation planning without execution', () => {
    const runtime = buildMonthlyFundingTracePlanningRuntime(fixture, planFixture);
    const contract = selectMonthlyCapacityFundingTraceImplementationContract(
      runtime.tracePackageCloseout,
      runtime.tracePlan
    );
    const summary = selectMonthlyCapacityFundingTraceImplementationSummary(contract);
    const blockedSummary = selectMonthlyCapacityFundingTraceImplementationSummary({
      ...contract,
      status: 'blocked' as const,
      recommendationCandidateId: null,
      plannedSourceGroups: []
    ...
    expect(summary).toMatchObject({
      ...
      plannedSectionCount: 5,
      plannedSourceGroupCount: 6,
      nextBroadStep: 'fundingTraceRuntimeExecution',
      plannedTrace: null,
      saved: false
    });
    expect(summary.boundary).toContain('without producing trace content');
    expect(blockedSummary).toMatchObject({
      status: 'blocked',
      recommendationCandidateId: null,
      plannedSourceGroupCount: 0,
      nextBroadStep: 'capacityInputsFirst',
      plannedTrace: null,
      saved: false
    });
    expect(createPlanFile(planFixture).plan).not.toHaveProperty('monthlyCapacityFundingTraceImplementationSummary');
  });

  it('closes out funding trace implementation planning without runtime trace output', () => {
    const runtime = buildMonthlyFundingTracePlanningRuntime(fixture, planFixture);
    const contract = selectMonthlyCapacityFundingTraceImplementationContract(
      runtime.tracePackageCloseout,
      runtime.tracePlan
    ...
    const closeout = selectMonthlyCapacityFundingTraceImplementationCloseout(contract);
    const blockedCloseout = selectMonthlyCapacityFundingTraceImplementationCloseout({
      ...contract,
      status: 'blocked' as const,
      recommendationCandidateId: null,
      plannedSourceGroups: []
    ...
    expect(closeout.boundary).toContain('trace runtime output');
    expect(blockedCloseout).toMatchObject({
      status: 'blocked',
      recommendationCandidateId: null,
      nextBroadStep: 'capacityInputsFirst'
    });
    expect(createPlanFile(planFixture).plan).not.toHaveProperty('monthlyCapacityFundingTraceImplementationCloseout');
  });

  it('checks funding trace implementation examples without runtime trace output', () => {
    const runtime = buildMonthlyFundingTracePlanningRuntime(fixture, planFixture);
    const contract = selectMonthlyCapacityFundingTraceImplementationContract(
      runtime.tracePackageCloseout,
      runtime.tracePlan
    ...
    const closeout = selectMonthlyCapacityFundingTraceImplementationCloseout(contract);
    const readiness = selectMonthlyCapacityFundingTraceImplementationExampleReadiness([closeout, closeout]);
    const blockedReadiness = selectMonthlyCapacityFundingTraceImplementationExampleReadiness([
      closeout,
      { ...closeout, status: 'blocked' as const, recommendationCandidateId: null }
    ]);
    ...
    expect(readiness.boundary).toContain('without producing trace content');
    expect(createPlanFile(planFixture).plan).not.toHaveProperty('monthlyCapacityFundingTraceImplementationExampleReadiness');
  });

  it('closes out the funding trace implementation planning package', () => {
    const runtime = buildMonthlyFundingTracePlanningRuntime(fixture, planFixture);
    const contract = selectMonthlyCapacityFundingTraceImplementationContract(
      runtime.tracePackageCloseout,
      runtime.tracePlan
    ...
    const exampleReadiness = selectMonthlyCapacityFundingTraceImplementationExampleReadiness([closeout]);
    const packageCloseout = selectMonthlyCapacityFundingTraceImplementationPackageCloseout(closeout, exampleReadiness);
    const blockedPackageCloseout = selectMonthlyCapacityFundingTraceImplementationPackageCloseout(
      closeout,
      selectMonthlyCapacityFundingTraceImplementationExampleReadiness([{ ...closeout, status: 'blocked' as const }])
    );
