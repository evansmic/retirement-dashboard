...
describe('Results overview structure', () => {
  ...
  it('explains bounded optimizer output without advice or saved-output language', () => {
    ...
    expect(appSource).toContain('Pilot readiness verification');
    expect(appSource).toContain('pilotReadinessChecklist.verificationRows');
    expect(appSource).toContain('No-go signals');
  ...
  it('keeps the normal Details evidence bands compact before optimizer work', () => {
    ...
    expect(detailsPanel).toContain('<ScenarioDecisionSummaryPanel');
    expect(detailsPanel).toContain('<BenefitTimingReadinessPanel');
    expect(detailsPanel).toContain('<SpendingStressPanel');
    expect(detailsPanel).toContain('<SequencingEvidenceReviewPanel');
    expect(detailsPanel).toContain('<BoundedOptimizerPanel loading={loading} summary={boundedOptimizer} variant="compact" />');
  ...
  it('keeps save and report actions distinct in the consumer UI', () => {
    ...
    expect(appSource).toContain('Compared with');
    expect(appSource).toContain('Why it matters');
    expect(appSource).toContain('Trade-off');
    expect(appSource).toContain('Scenario decision summary');
    expect(appSource).toContain('Best current alternate');
    expect(appSource).toContain('Sequencing evidence check');
    expect(appSource).toContain('Public download');
  ...
  it('exports year-by-year CSV from annual detail rows without changing saved plan copy', () => {
    const masterDownloadStart = appSource.indexOf('function downloadMasterDetailCsv()');
    const masterDownloadEnd = appSource.indexOf('useEffect(() => {', masterDownloadStart);
    const masterDownloadSource = appSource.slice(masterDownloadStart, masterDownloadEnd);
    ...
    expect(appSource).toContain('MasterDetailCsvRow');
    expect(appSource).toContain('Registered withdrawals');
    expect(appSource).toContain('Estate before tax');
    expect(appSource).toContain('Money-flow check');
    expect(appSource).toContain('This is a local results');
    expect(masterDownloadSource).toContain('masterDetailRowsToCsv(masterDetailRowsForDownload)');
    expect(masterDownloadSource).not.toContain('masterDetailRowsToCsv(masterDetailRows)');
    expect(appSource).not.toContain("id: 'betaSequencingReviewAmount'");
